...
contract dBank {
  ...
  function deposit() payable public {
    ...
    require(msg.value >= 1e16, 'Error, deposit must be >= than .01 ETH' );
    //increase msg.sender ether deposit balance
    etherBalanceOf[msg.sender] = etherBalanceOf[msg.sender] + msg.value;
    
    //start msg.sender hodling time / record the start time of deposit
    depositStart[msg.sender] = depositStart[msg.sender] + block.timestamp;

    //set msg.sender deposit status to true / activate deposit status
  ...
  function withdraw() public {
    ...
    //send interest in tokens to user // the bank mints token dependent upon interest earned
